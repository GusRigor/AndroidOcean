...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btAddLanguage = findViewById<Button>(R.id.btAddLanguage)
        ...
        val items = listOf(kotlin, java).toMutableList()
        ...
        btAddLanguage.setOnClickListener {
            items.add(cplusplus)
            rvLinguagemProgramacao.adapter!!.run{
                //notifyDataSetChanged()
                notifyItemInserted(items.lastIndex)
            }
            rvLinguagemProgramacao.scrollToPosition(items.lastIndex)
